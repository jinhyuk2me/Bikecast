...
수집된 데이터는 **AWS RDS MySQL 서버**의 `rental` 테이블을 중심으로 저장·관리되고 있으며, 분석 및 모델 학습 단계에서는 SQL 쿼리를 통해 필요한 데이터를 추출한 뒤, Pandas 기반의 **DataFrame 형태로 변환**하여 전처리 및 feature 생성이 수행됩니다.

각 feature는 대여소별·시간대별 수요를 설명할 수 있도록 구성되며, 최종적으로는 **[대여 시각, 대여소 위치]를 Primary Key로 하는 학습용 데이터셋**이 구성됩니다.

> 📌 해당 학습용 데이터셋은 2024년 한 해 동안의 **전체 이용기록 44,543,263건**을 기반으로 하며, 이는 모델의 학습 정밀도와 분석 신뢰도를 뒷받침하는 중요한 기반이 됩니다.
